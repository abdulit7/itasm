...
#         self.asset_category = ft.Dropdown(
#             options=[
#                 ft.dropdown.Option(key="default", text="Select Category"),  # Default option
#             ],
#             width=200,
#             hint_text="Select Category",
#             on_change=lambda e: self.load_categories()  # Reload assets when category changes
#         )
...
#         # Load initial assets
#         self.load_categories()  # Load categories first
#         self.load_assets()
...
#             cursor.execute("SELECT id, name FROM category WHERE type = 'Asset'")  # Adjusted query to use 'name'
#             categories = cursor.fetchall()

#             # Populate dropdown options with id as key and name as text
#             for category in categories:
#                 self.asset_category.options.append(
#                     ft.dropdown.Option(key=str(category['id']), text=category['name'])
#                 )
#             self.asset_category.update()

#         except Error as e:
#             self.page.snack_bar = ft.SnackBar(
#                 content=ft.Text(f"Error fetching categories: {e}"),
#                 duration=4000
#             )
...
class AssetPage(ft.Container):
    def __init__(self, page: ft.Page):
        ...
        # Initialize category dropdown with a default option
        self.asset_category = ft.Dropdown(
            options=[
                ft.dropdown.Option(key="default", text="Select Category")  # Default option
            ],
        ...
        # Defer page update and category loading to ensure the control is added first
        self.page.add(self)
        self.load_categories()  # Load categories after adding to page
        self.load_assets()
    ...
    def load_categories(self):
        ...
        try:
            ...
            self.asset_category.update()  # Safe to update now that it's in the control tree
    ...
    def on_category_change(self, e):
        """Handle category change event to filter assets."""
        selected_category_id = self.asset_category.value
        if selected_category_id and selected_category_id != "default":
            self.load_assets(category_id=selected_category_id)  # Pass category_id to filter
        else:
            self.load_assets()  # Reload all assets if default is selected

    def load_assets(self, category_id=None):
        """Reload asset data from the database and update the UI with optional category filter."""
        self.asset_add = []
        db_config = {
            "host": "200.200.201.100",
            "user": "root",
        ...
        try:
            ...
            if category_id:
                ...
                params = (category_id,)
            cursor_assets.execute(query, params)
            assets = cursor_assets.fetchall()
            cursor_assets.close()
            ...
            for asset in assets:
                cursor_images = conn.cursor(dictionary=True, buffered=True)
                cursor_images.execute("SELECT image_data FROM asset_images WHERE asset_id = %s", (asset['id'],))
                image_results = cursor_images.fetchall()
