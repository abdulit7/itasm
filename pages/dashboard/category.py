...
class Category(ft.Container):
    ...
    def _get_total_items(self, category_id: int, connection) -> int:
        ...
        try:
            ...
            # Count components (using category_id)
            ...
            # Note: assets and consumables are not counted due to missing category_id
            print(f"Category {category_id} has {total_items} components (assets and consumables not counted due to missing category_id)")
            return total_items
        except mysql.connector.Error as error:
            print(f"Error calculating total items: {error}")
            return 0
