...
#             options=[ft.dropdown.Option(key="default", text="All")],
...
#             self.component_category.options.insert(0, ft.dropdown.Option(key="default", text="All"))
#             if self.dialog.open:
#                 self.page.update()
...
#                 self.error_popup.content = ft.Text(f"Component with serial number {serial_number} already exists.")
#                 self.error_popup.open = True
#                 self.page.overlay.append(self.error_popup)
#                 self.page.update()
#                 return
#             selected_category_id = int(category_id) if category_id and category_id != "default" else None
#             else:
#                 cursor.execute("""
...
class ComponentDialog:
    def __init__(self, page: ft.Page, parent=None):
        ...
        self.component_category = ft.Dropdown(
            ...
            leading_icon=ft.Icons.SEARCH,
            hint_text="Select Category",
            options=[ft.dropdown.Option(key="default", text="Select Category")],  # Changed "All" to "Select Category"
            on_change=self.update_category
        )
    ...
    def load_categories(self):
        try:
            ...
            categories = cursor.fetchall()
            self.component_category.options = [ft.dropdown.Option(key=str(cat[0]), text=cat[1]) for cat in categories]
            self.component_category.options.insert(0, ft.dropdown.Option(key="default", text="Select Category"))  # Ensure "Select Category" is first
            if self.dialog.open:
    ...
    def update_category(self, e):
        # Optional: Add logic to handle category change (e.g., validation or filtering)
        pass
    ...
    def submit_form(self, event):
        ...
        try:
            ...
            """, (selected_category_id, model, serial_number, company, location, purchase_date, status))
            component_id = cursor.lastrowid
            ...
            cursor.execute("""
                INSERT INTO asset_history (table_type, entity_id, data_json, action)
                VALUES (%s, %s, %s, %s)
            """, ('components', component_id, f'{{"model":"{model}", "serial_number":"{serial_number}", "status":"{status}"}}', 'Added'))
