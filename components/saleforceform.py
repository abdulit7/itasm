...
#         )
#         self.device_image = ft.FilePicker(on_result=self.handle_device_image)
...
#         )
#         self.device_bill = ft.FilePicker(on_result=self.handle_bill_image)
...
#             try:
#                 # Read file data from path (desktop)
#                 if file.path:
#                     with open(file.path, 'rb') as f:
#                         file_data = {"name": file.name, "data": f.read()}
#                 else:
#                     # Web/Pyodide: Assume data is provided (requires Flet version support)
#                     self.error_popup.content = ft.Text(f"File {file.name} cannot be read in this environment.")
#                     self.error_popup.open = True
#                     self.page.update()
#                     continue
#                 if len(file_data["data"]) > 50 * 1024 * 1024:
#                     self.error_popup.content = ft.Text(f"File {file.name} exceeds 50 MB.")
#                     self.error_popup.open = True
#                     self.page.update()
#                     continue
#                 self.attached_images.append(file_data)
#             except Exception as e:
#                 self.error_popup.content = ft.Text(f"Error reading file {file.name}: {e}")
#                 self.error_popup.open = True
#                 self.page.update()
#         self.device_image_button.text = f"{len(self.attached_images)} image(s) selected"
#         self.page.update()
...
#                         file_data = {"name": file.name, "data": f.read()}
#                 else:
#                     # Web/Pyodide: Assume data is provided
#                     self.error_popup.content = ft.Text(f"File {file.name} cannot be read in this environment.")
#                     self.error_popup.open = True
#                     self.page.update()
#                     continue
...
#                 self.error_popup.open = True
#                 self.page.update()
#         self.device_bill_button.text = f"{len(self.attached_bills)} bill(s) selected"
#         self.page.update()
